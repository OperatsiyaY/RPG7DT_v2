...
var vector FlashTracerOffset; // положение трейла относительно снаряда
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	{
		Spawn(class'RPG7DT_v2.RpgExplosion',,,HitLocation + HitNormal*20,rotator(HitNormal));
		Spawn(ExplosionDecal,self,,HitLocation, rotator(-HitNormal));
	}

	//FlashTracer.Destroyed(); // при взрыве уничтожаем флэш трейсер
	//FlashTracer.Destroy();
	BlowUp(HitLocation);
	Destroy();
...
simulated function PostBeginPlay()
{
	...
	if ( Level.NetMode != NM_DedicatedServer)
	{
		...
		FlashTracer = Spawn(class'RPGTracer', self,, Location + X*FlashTracerOffset.X + Y*FlashTracerOffset.Y + Z*FlashTracerOffset.Z, Rotation);
		AttachToBone(FlashTracer, 'Grenade');
		FlashRotation.Pitch = -16384;
		FlashTracer.SetRelativeRotation(FlashRotation);
		MoveVect.Z = 70;
		FlashTracer.SetRelativeLocation(MoveVect);
		
		//FlashTracer.SetBase(self);
		// Corona = Spawn(class'RPG7DT.KFRPGCorona',self);
	}
...
simulated function Destroyed()
{
	...
	if ( FlashTracer != None )
	{
		FlashTracer.HandleOwnerDestroyed();
	}
...
simulated function Tick( float DeltaTime )
{
	...
	LoopAnim('Spin',1.0);
	//FlashTracer.SetLocation(Self.Location); // тянем за собой флэш трейсер
}
...
defaultproperties
{
	...
	///*********мои добавления************
	 // mMuzFlashClass=Class'RPG7DT.RGPRocketTrail'
     // TrailOffset=(X=-15.000000)
	...
	///***********************************
     ...
     OffsetTime=3.500000
     ExplosionSound=Sound'RPG7DTv2_A.RPGExplode'
     DisintegrateSound=Sound'Inf_Weapons.panzerfaust60.faust_explode_distant02'
     ArmDistSquared=250000.000000
     ImpactDamageType=Class'RPG7DT_v2.DamTypeRpgRocketImpact'
     ImpactDamage=250
     AmbientVolumeScale=5.000000
     bTrueBallistics=False
     Speed=50//3000.000000
     MaxSpeed=60//4000.000000
     Damage=950.000000
     DamageRadius=600.000000
     MomentumTransfer=2000.000000
     MyDamageType=Class'RPG7DT_v2.DamTypeRPG'
     ExplosionDecal=Class'RPG7DT_v2.RPGMarkDirt'
     ExplosionDecalSnow=Class'RPG7DT_v2.RPGMarkDirt'
     LightHue=25
     LightSaturation=100
     LightBrightness=200.000000
     LightRadius=15.000000
     DrawType=DT_Mesh
     Mesh=SkeletalMesh'RPG7DTv2_A.RPG_GrenadeDTMesh'
     AmbientSound=Sound'RPG7DTv2_A.RPGFly'
     LifeSpan=10.000000
     DrawScale=0.700000
     bUnlit=False
     SoundVolume=192
	 ...
	 FlashTracerOffset=(X=0.0,Y=0.0,Z=0.0)
}
